...
import escapeString from 'sitterfied-web/utils/escape-string'
...
var Select2Component = Ember.Component.extend({
  ...
  style: escapeString("display: hidden;"),
  ...
  didInsertElement: function() {
    ...
    options.context = this.get('queryContext');
    options.allowClear = this.get('allowClear');
    options.minimumInputLength = this.get('minimumInputLength');
    options.formatInputTooShort = this.get('formatInputTooShort');
    ...
    options.formatResult = function(item) {
      ...
      if (self.get('resultTemplate')) {
        output = self.get('resultTemplate')(item);
      } else {
    ...
    options.formatSelection = function(item) {
      ...
      if (self.get('selectionTemplate')) {
        return self.get('selectionTemplate')(item);
      } else {
     ...
     */
    options.query = this.get('query') || function(query) {
      var select2 = this;
    ...
    /*
      ...
      and one time directly when setting up the select2 plugin even without
      "oVP".  (but with empty value, which will just skip the method)
  ...
  valueChanged: Ember.observer(function() {
    ...
    if (optionValuePath) {
      // when there is a optionValuePath, the external value is a primitive
      // value so use the "val" method

      // Don't update the value if it's the same.
      if (_.isEqual(value, this._select.select2("val")) ) {
        return;
      }
      this._select.select2("val", value);
    } else {
      // Don't update the value if it's the same.
      if (_.isEqual(value, this._select.select2("data")) ) {
        return;
      }
  ...
  contentChanged: Ember.observer(function() {
    this.valueChanged();
  }, "content.[]", "content.@each.text"),

  disabledChanged: Ember.observer(function() {
    if (!this._select) { return }
    this._select.select2("enable", !this.get("disabled"));
  }, "disabled"),

  placeholderChanged: Ember.observer(function() {
    if (!this._select) { return }
